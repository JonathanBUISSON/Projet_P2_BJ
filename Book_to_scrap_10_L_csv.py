...
for i in range(1,51):
  url_category="https://books.toscrape.com/catalogue/category/books/poetry_"+str(i)+"/index.html" 
...
for url in book_link:
    ...
    if response_book.ok:
        ...
        #book_data.append([cat, title, review_rating, UPC, Product_type, Price_excluding_tax, Price_including_tax, Tax, Availability, Numbers_of_review, product_description])
            #Ce book_data ci-dessus est celui que l on a eu la dernière fois il fallait l'adapter, ce que j'ai fait dans le book_data ci-dessous.
        ...
        with open(category_name + '.csv', mode='w', encoding= 'UTF-8') as csv_file:
